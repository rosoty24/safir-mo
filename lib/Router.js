...
Router.route('/category/:id', {
    ...
    waitOn: function() {
       var name = this.params.id;
        name = name.replace(/-/g, " ");
        return [Meteor.subscribe("productsCategory", -1, name), Meteor.subscribe("attributeCategory", name), TAPi18n.subscribe('categories'), TAPi18n.subscribe('categoryParent_tags', name), Meteor.subscribe("attributeCategory", name), TAPi18n.subscribe('categoryTags', name), Meteor.subscribe("favoriteCategory", name)];
...
Router.route('/details/:id', {
    ...
    waitOn: function() {
        ...
        title = title.replace(/\(eaccentgrave\)/g, "è");
        title = title.replace(/\(hyphen\)/g, "–");
        return [Meteor.subscribe("productsDetails", -1, title), Meteor.subscribe("attributeProDetails", -1, title), TAPi18n.subscribe("detailsParent_tags", title), TAPi18n.subscribe("detailsTags", title), TAPi18n.subscribe("parentattrProDetails")];
...
Router.route('/advanced', {
    ...
    template: 'listproducts',
    waitOn: function() {
        return [TAPi18n.subscribe('productsCategory', -1), TAPi18n.subscribe('categories'), TAPi18n.subscribe('parent_tags'), TAPi18n.subscribe('tags')];
    },
    data: function() {
        var list_categories = [];
        ...
        */
        ...
        if (list_tags.length > 0) {
            var result = products.find({ category: { $in: list_categories }, Brand: { $in: list_brand }, "tags.value": { $in: list_tags } }, { limit: Session.get('querylimit') });
        } else {
            var result = products.find({ category: { $in: list_categories }, Brand: { $in: list_brand } }, { limit: Session.get('querylimit') });
        }
...
Router.route('/webzinelisting', {
    name: 'webzinelisting',
    waitOn: function() {
        return [Meteor.subscribe('contents'), Meteor.subscribe('banner')];
    }
});
...
>>>>>>> 214c0d5a3be27875edeab6ba9885885f100ff7a2
    },
    waitOn: function() {
...
Router.route('/tutolisting/:_id', {
    ...
    data: function() {
        ...
        //alert('title='+title);
        console.log('mytuto:' + name);
        return categories.findOne({ "title": { $regex: new RegExp("^" + name, "i") } });
    },
    waitOn: function() {
        return [Meteor.subscribe("contents")];
    }
